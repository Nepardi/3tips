...
def strip_product_codes(text):
    ...
    text = re.sub(r'(?:Item\s*(?:No\.)?|No\.|#)\s*[A-Za-z]*\d+[A-Za-z0-9\-]*', '', text)
    # Clean up
    text = re.sub(r'  +', ' ', text)
...
for tip in tips:
   ...
   # 2. Warn if no brand references
    has_brand = any(b in text_lower for b in VALID_BRANDS)
    if not has_brand:
        print(f"  WARNING: tip '{title}' has no brand references")
    
   # 3. Warn if text is too short (< 50 words)
    word_count = len(text_lower.split())
    if word_count < 50:
        print(f"  WARNING: tip '{title}' is only {word_count} words")
        # Strip product codes from text and title
    if 'text' in tip:
        tip['text'] = strip_product_codes(tip['text'])
    if 'title' in tip:
        tip['title'] = strip_product_codes(tip['title'])
...
<link rel="alternate" type="application/rss+xml" title="Scale Model Tips RSS" href="/rss.xml">
<style>
:root {{
  --primary: #6d4aff;
  --primary-dark: #573dd4;
...
These tips are automatically generated by an AI language model via Ollama. While the content aims to be helpful, it may contain errors or outdated information. Always verify advice against reliable sources before applying it to your projects.<br><br>
<strong>All images</strong> are original photographs taken by the project creator from actual scale models. However, <strong>the card images may not necessarily illustrate the specific tip on that card</strong> — they are selected randomly from the available photo collection for visual variety.
</div>
</div>
<div class="actions">
...
# ========== GENERATE RSS FEED ==========
rss_path = 'rss.xml'
site_url = 'https://scalemodeltips.eu'
site_title = 'Scale Model Tips'
site_description = 'Daily scale model building tips generated by AI'
...
ET.SubElement(channel, 'generator').text = 'Scale Model Tips Generator'

tip_timestamp = datetime.now().strftime('%Y%m%d%H%M%S')

for i, tip in enumerate(tips, 1):
    item = ET.SubElement(channel, 'item')
    title_text = f"{topic_display}: {tip.get('title', 'Untitled Tip')}"
    ET.SubElement(item, 'title').text = title_text
    full_text = tip.get('text', '')
    desc_text = full_text[:200] + '...' if len(full_text) > 200 else full_text
    ET.SubElement(item, 'description').text = desc_text
    ET.SubElement(item, 'pubDate').text = current_date
    guid = ET.SubElement(item, 'guid')
    guid.set('isPermaLink', 'false')
    guid.text = f"{site_url}/?tip={tip_timestamp}_{i}"

tree = ET.ElementTree(rss)
ET.indent(tree, space='  ')
tree.write(rss_path, encoding='utf-8', xml_declaration=True)
print(f"RSS feed updated at {rss_path}")
